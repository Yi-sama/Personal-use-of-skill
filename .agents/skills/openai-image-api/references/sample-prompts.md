...
These prompt recipes are for the `openai-image-api` workflow in `scripts/image_api.py`.
...
Execution details such as explicit CLI flags, `quality`, `input_fidelity`, masks, output formats, local output paths, `--base-url`, `--base-url-env`, `--api-key-env`, and `--codex-home` belong in the CLI command. For transparent images, prompt for a flat chroma-key background and remove it locally with `python "<skill-dir>/scripts/remove_chroma_key.py"`; ask before switching to true CLI transparency with `gpt-image-1.5`.

CLI model notes:
- `gpt-image-2` is the default Image API model for new workflows.
- `gpt-image-2` supports `quality` values `low`, `medium`, `high`, and `auto`.
- For 4K-style `gpt-image-2` output, use `3840x2160` or `2160x3840`.
- If transparent output needs true CLI transparency, ask before using `gpt-image-1.5` unless the user already explicitly requested `gpt-image-1.5`. Explain that chroma-key removal is the default path, but `gpt-image-2` does not support `background=transparent`.
- Do not set `input_fidelity` with `gpt-image-2`; image inputs already use high fidelity.

Text policy for examples:
- `no text` constraints apply only when the requested asset is a background, texture, icon, cutout, or visual scene that should not contain copy.
- If the user asks for poster copy, invitation text, labels, titles, Chinese layout, or complete typography, replace `no text` with a `Text (verbatim): ...` block and text-bearing constraints.
