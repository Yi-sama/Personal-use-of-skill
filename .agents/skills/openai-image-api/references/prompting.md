...
These prompting principles apply to the `openai-image-api` workflow in `scripts/image_api.py`.
...
## Text in images
- If the user requests visible text, poster typography, invitation copy, labels, titles, Chinese layout, or complete text layout, treat it as a text-bearing image request.
- Do not add `no text`, `no typography`, `blank background`, or `leave room for copy` unless the user explicitly asks for a text-free asset.
- Put all literal text in a dedicated block:

  Text (verbatim):
  - "<exact text 1>"
  - "<exact text 2>"

- Add constraints: render the listed text directly inside the image; preserve every character; no omitted text; no invented text; no extra captions; no garbled characters.
- Specify typography, placement, hierarchy, color, and approximate size for each text group when relevant.
- Spell uncommon words letter-by-letter if accuracy matters.
- Use `medium` or `high` quality for small text, dense infographics, data-heavy slides, multi-font layouts, legends, axes, footnotes, Chinese text, and final poster assets.
- Default to direct in-image rendering for text-bearing requests, including Chinese poster/layout requests. Do not switch to a text-free background plus local overlay solely for legibility or Chinese text.
- Mention local/programmatic text overlay only when the user explicitly asks for editable production typography, pixel-perfect copy, exact font/brand control, or separately composited text, or when direct generation fails text validation and the user accepts that tradeoff.
...
## Transparent images
- Use `scripts/image_api.py` with chroma-key removal first for transparent-image requests. If the subject is clearly too complex for chroma-key removal, explain the true-transparent model path and ask before switching.
...
- Use CLI `gpt-image-1.5 --background transparent --output-format png` only after the user explicitly confirms the model path, or when the user already explicitly requested `gpt-image-1.5`. Ask first for true/native transparency requests, failed chroma-key validation, or complex transparent subjects such as hair, fur, glass, smoke, liquids, translucent materials, reflective objects, or soft shadows.

## CLI execution controls
- `quality`, `input_fidelity`, explicit masks, output format, output paths, `--base-url`, `--base-url-env`, `--api-key-env`, and `--codex-home` are CLI execution controls.
- Put execution controls in command arguments, not in the natural-language prompt.
- See `SKILL.md` and `references/official-image-api.md` for those controls.
- In `openai-image-api`, `gpt-image-2` is the default Image API model. It supports `quality=low|medium|high|auto`; use `low` for fast drafts and thumbnails, and move to `medium`, `high`, or `auto` for final assets.
