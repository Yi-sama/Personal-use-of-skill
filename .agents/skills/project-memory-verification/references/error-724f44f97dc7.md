...
Use this case when calling the FastCtx MCP `read`, `grep`, `glob`, or `replace` tools; when a FastCtx tool appears installed but cannot be invoked through a generic tool wrapper; when a result is `Partial`; or when a path, regex, replacement, or write-guard diagnostic is returned.
...
1. Confirm installation and registration with `fastctx status`. Treat successful config parsing, applied state, installed binary, MCP handshake, and guidance checks as separate evidence.
